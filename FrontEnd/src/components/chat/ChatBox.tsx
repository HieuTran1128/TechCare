...
/** Tạo roomId duy nhất cho cuộc trò chuyện giữa hai user (sắp xếp để đảm bảo nhất quán). */
const getRoomId = (a: string, b: string) => [a, b].sort().join('__');

/** Component hiển thị avatar: dùng ảnh nếu có, fallback về chữ cái đầu tên. */
...
export const ChatBox: React.FC<ChatBoxProps> = ({ currentUser }) => {
  ...
  /**
   * Cập nhật hoặc thêm mới một conversation trong danh sách.
   * Nếu đã tồn tại thì merge patch, nếu chưa có thì tạo mới và sắp xếp theo tin nhắn mới nhất.
   */
  ...
  /** Tải thêm tin nhắn cũ hơn (phân trang), giữ nguyên vị trí scroll sau khi load. */
  ...
  /** Chọn user để mở cuộc trò chuyện. */
  const handleSelectUser = (userId: string) => {
    setSelectedUserId(userId);
  };

  /** Gửi tin nhắn đến user đang được chọn và reset input. */
  const handleSend = () => {
    if (!socket || !selectedUserId) return;
  ...
  /** Cập nhật input và phát sự kiện typing, tự động dừng sau 900ms không gõ. */
  ...
  /**
   * Danh sách sidebar: merge conversation history và online users,
   * lọc theo searchTerm và sắp xếp theo tin nhắn mới nhất.
   */
