...
interface InventoryKpiData {
  groupBy: GroupBy;
  summary: KpiSummary;
  periodSummary: PeriodRow[];
  topConsumptionParts: TopPart[];
  topImportParts: TopImportPart[];
}
...
const InventoryKPI: React.FC = () => {
  ...
  const fetchKpi = async () => {
    try {
      setLoading(true);
      setError('');
      const res = await axios.get<InventoryKpiData>(`${API_BASE}/parts/kpi`, {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `kpi-kho-${startDate}-${endDate}.csv`;
    a.click();
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border rounded-2xl p-4 flex flex-wrap gap-3 items-end shadow-sm">
        <div>
          ...
          <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="mt-1 block border rounded-xl px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="text-xs text-slate-500">Đến ngày</label>
          <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="mt-1 block border rounded-xl px-3 py-2 text-sm" />
        </div>
        <div>
        ...
        <button onClick={fetchKpi} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 text-white text-sm font-semibold">
          <Filter size={14} /> Áp dụng
        </button>
        <button onClick={exportCSV} disabled={!data} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 text-white text-sm font-semibold disabled:opacity-40">
            ...
            {summaryCards.map((card) => {
              ...
              return (
                <div key={card.label} className="bg-white border rounded-2xl p-4 shadow-sm flex items-center justify-between gap-3">
                  ...
                  <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${card.tone} text-white grid place-items-center shrink-0`}>
                    <Icon size={18} />
                  </div>
                </div>
              );
                ...
                <tbody>
                  {data.topConsumptionParts.map((p, i) => (
                    <tr key={p.partId} className="border-t border-slate-100 hover:bg-slate-50/60">
                      ...
                      <td className={`px-3 py-2 text-right font-semibold ${p.currentStock <= 0 ? 'text-rose-600' : p.currentStock <= 5 ? 'text-amber-600' : 'text-slate-700'}`}>
                        {fmtNum(p.currentStock)}
                      </td>
                    </tr>
                  ))}
                  {data.topConsumptionParts.length === 0 && (
                    <tr><td colSpan={7} className="px-3 py-4 text-center text-slate-400 text-sm">Chưa có dữ liệu xuất kho</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                    ...
                    return (
                      <tr key={p.partId} className="border-t border-slate-100 hover:bg-slate-50/60">
                        ...
                        <td className="px-3 py-2 text-right">
                          {fmt(p.sellPrice)}
                          <span className={`ml-1 text-xs ${marginPct >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                  ...
                  {data.topImportParts.length === 0 && (
                    <tr><td colSpan={8} className="px-3 py-4 text-center text-slate-400 text-sm">Chưa có dữ liệu nhập kho</td></tr>
                  )}
                </tbody>
              </table>
            </div>
