    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Bảng Lương Nhân Sự</h1>
                    <p className="text-slate-500 text-sm mt-1">Quản lý giờ làm và thiết lập mức lương theo giờ (1 ca = 4 giờ)</p>
                </div>
            ...
            <div className="bg-white/60 dark:bg-slate-800/40 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700 shadow-lg overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-10 text-slate-500">Không tìm thấy dữ liệu lương.</div>
                ) : (
                        ...
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            <thead className="bg-slate-100/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-semibold text-center">Số giờ làm</th>
                                    <th className="px-6 py-4 font-semibold text-right">Lương / Giờ (VNĐ)</th>
                                    <th className="px-6 py-4 font-semibold text-right">Tổng nhận (VNĐ)</th>
                                    <th className="px-6 py-4 font-semibold text-center">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-200/60 dark:divide-slate-700/60">
                                {filteredSalaries.map((s) => (
                                    <tr key={s.userId} className="hover:bg-white/40 dark:hover:bg-slate-800/40 transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-900 dark:text-slate-100">{s.fullName}</td>
                                        <td className="px-6 py-4 text-slate-500 capitalize">{s.role}</td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 py-1 px-3 rounded-full font-medium">
                                                {s.totalHours} giờ
                                            </span>
                                            <div className="text-xs text-slate-400 mt-1">({s.totalShifts} ca)</div>
                                        </td>
                                        <td className="px-6 py-4 text-right font-medium">{(s.hourlyRate || 0).toLocaleString()} ₫</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-800">
                        <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-800">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Cập nhật mức lương</h3>
                        ...
                        <div className="p-5 space-y-4">
                            <div>
                                ...
                                <p className="font-semibold text-slate-900 dark:text-white">
                                    {editingUser.fullName} <span className="text-xs font-normal text-slate-400 uppercase ml-2">({editingUser.role})</span>
                                </p>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                                    <p className="text-sm text-blue-800 dark:text-blue-300">
                                        Nhân viên này đã đăng ký <strong>{editingUser.totalShifts} ca</strong> (tương đương <strong>{editingUser.totalHours} giờ</strong> làm việc).
                                    </p>
                                </div>
                            ...
                            <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                ...
                                <span className="text-xl font-bold text-indigo-600">{(editRate * editingUser.totalHours).toLocaleString()} ₫</span>
                            </div>
                        </div>
